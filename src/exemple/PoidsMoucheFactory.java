...
public class PoidsMoucheFactory {
	...
	/**
	 * Instancie un  {@link IObjetSpatial} seulement si listObjSpa ne contient pas une instance avec le même pays
	 * et l'ajoute à sa liste d'{@link ObjetSpatialPartage}.
	 * @param pValeur
	 * @param name 
	 * 
	 */
	public IObjetSpatial getObjetSpatial(String paysOrganisation, String name) {
		//Si la liste d'IObjetSpatial ne contient pas d'instance dont l'attribut paysOrganisation est égal au name donné en paramètre
		if(!listObjSpa.containsKey(paysOrganisation)) {	
			//Détection d'un debris
			if (name.contains(DEBRIS)) {
				//instanciation d'une nouvelle instance d'ObjetSpatialDebris
				listObjSpa.put(paysOrganisation, new ObjetSpacialDebris(paysOrganisation));	
			} else {
				//instanciation d'une nouvelle instance d'ObjetSpatial
